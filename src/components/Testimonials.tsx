...
export default function Testimonials({ items }: Props) {
  ...
  return (
    <div className="space-y-8">
      {/* Featured */}
      <figure className="relative rounded-xl border border-white/10 p-6 md:p-8 bg-white/[.03] shadow-md">
        {featured.logo && (
          <div className="pointer-events-none absolute inset-0 z-0">
            <div className="absolute right-3 opacity-10 md:opacity-20" style={{ bottom: "-24px" }}>
              <span className="relative block h-32 w-56 md:h-44 md:w-80">
                <Image src={featured.logo} alt="" fill className="object-contain" />
              </span>
            </div>
          </div>
        )}
        <div className="h-1 w-full mb-3" style={{ background: "linear-gradient(90deg, var(--gold-start), var(--gold-end))" }} />
        <div className="pointer-events-none absolute -top-1 right-3 opacity-10" aria-hidden>
        ...
        <div className="flex items-start gap-4 relative z-10">
          {featured.avatar && (
            <div className="relative h-14 w-14 shrink-0 rounded-full overflow-hidden border-2" style={{ borderColor: "var(--gold-start)" }}>
              <Image src={featured.avatar} alt={featured.name} fill className="object-cover" />
            </div>
          )}
          <blockquote className="text-lg md:text-xl leading-relaxed flex-1">
            <span aria-hidden className="inline-block align-top mr-2" style={{ color: "var(--gold-start)" }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M7.17 6A5 5 0 0 0 2 11v7h7v-7H6.83A3 3 0 0 1 9 8.17V6H7.17Zm9 0A5 5 0 0 0 11 11v7h7v-7h-2.17A3 3 0 0 1 18 8.17V6h-1.83Z"/></svg>
            </span>
            “{featured.quote}”
          </blockquote>
        </div>
        <figcaption className="mt-4 flex flex-col items-start gap-1 text-sm text-soft-gray relative z-10">
          <span>
            — {featured.name}
          ...
          <div className="inline-flex items-center gap-1" aria-hidden style={{ color: "var(--gold-start)" }}>
            {Array.from({ length: 5 }).map((_, i) => (
              <svg key={i} width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.62L12 2 9.19 8.62 2 9.24l5.46 4.73L5.82 21z"/></svg>
            ))}
          </div>
        </figcaption>
      </figure>
